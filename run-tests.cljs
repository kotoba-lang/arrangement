(ns run-tests
  ...
  arrangement is the datom index both Workers query through. Its cljs half is smaller than its JVM half by reader conditionals, so this is an ADDITION to the JVM gate, not a replacement.
  ...
  (:require [cljs.test :as t]
            [arrangement.commit-delta-test]
            [arrangement.core-test]
            [arrangement.facade-test]
            [arrangement.lego-test]
            [arrangement.partitioned-test]
            [arrangement.range-index-test]
            [arrangement.source-test]))
...
(t/run-all-tests #"^arrangement\..*-test$")
